...
describe("Transpose", () => {
  ...
  it("square", () => {
    ...
    expect(transpose(["HEART", "EMBER", "ABUSE", "RESIN", "TREND"])).toEqual(expected);
  });

  it("rectangle", () => {
    const expected = ["FOBS", "RULE", "ATOP", "CLOT", "TIME", "UNIT", "RENT", "EDGE"];
    expect(transpose(["FRACTURE", "OUTLINED", "BLOOMING", "SEPTETTE"])).toEqual(expected);
  });

  it("triangle", () => {
    const expected = ["TEASER", " EASER", "  ASER", "   SER", "    ER", "     R"];
    expect(transpose(["T", "EE", "AAA", "SSSS", "EEEEE", "RRRRRR"])).toEqual(expected);
  });

  it("jagged triangle", () => {
    const expected = ["123456", "1 3456", "  3456", "  3 56", "    56", "    5"];

    expect(transpose(["11", "2", "3333", "444", "555555", "66666"])).toEqual(expected);
  });
